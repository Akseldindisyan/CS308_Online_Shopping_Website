...
-- Consolidate duplicate active carts before enforcing one active cart per owner.
WITH duplicate_user_carts AS (
    SELECT
        id,
        FIRST_VALUE(id) OVER (PARTITION BY user_id ORDER BY id) AS keep_id,
...
SET cart_id = duplicate.keep_id
FROM duplicate_user_carts duplicate
WHERE item.cart_id = duplicate.id AND duplicate.row_number > 1;
...
USING duplicate_user_carts duplicate
WHERE cart.id = duplicate.id AND duplicate.row_number > 1;

WITH duplicate_guest_carts AS (
    SELECT
        id,
        FIRST_VALUE(id) OVER (PARTITION BY guest_token ORDER BY id) AS keep_id,
...
UPDATE cart_item_entity item
SET cart_id = duplicate.keep_id
FROM duplicate_guest_carts duplicate
WHERE item.cart_id = duplicate.id AND duplicate.row_number > 1;
...
DELETE FROM cart_entity cart
USING duplicate_guest_carts duplicate
WHERE cart.id = duplicate.id AND duplicate.row_number > 1;

-- Moving cart contents can create repeated product rows; combine their quantities.
...
UPDATE cart_item_entity item
SET quantity = duplicate.total_quantity
FROM duplicate_items duplicate
